...
import { Palette, Video, Layers, Code, Lightbulb, Users } from "lucide-react";
...
import { getDownloadUrl } from "@/lib/uploadOnAWS";
const About = ({ introductionData, onStartProject }: AboutProps) => {
  ...
  const aboutStats = [
    ...
    {
      number: introductionData?.yearsOfExperience + "+",
      label: "Years Experience",
    },
    { number: introductionData?.numberOfClients + "+", label: "Happy Clients" },
    {
      number: introductionData?.clientSatisfaction + "%",
...
const handleDownloadCV = async () => {
  try {
    const key = introductionData?.resume;
    if (!key) {
    ...
    // Simply open the URL in a new tab - the ResponseContentDisposition will handle the download
  ...
  return (
    <section
      id="about"
      className="py-20 lg:py-32 bg-gradient-to-b from-background to-muted/20"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-24 lg:mb-32 text-center">
          ...
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto leading-relaxed">
            I specialize in creating comprehensive visual solutions that help
            brands stand out, communicate effectively, and connect with their
            audience through strategic design.
          </p>
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          <div className="space-y-8">
            <Badge
              variant="outline"
              className="text-purple-700 dark:text-purple-300 text-2xl"
            >
              👨‍💻 About Me
            </Badge>
            <h2 className="text-4xl lg:text-5xl font-bold text-foreground mb-6">
            ...
            <div className="space-y-6 text-muted-foreground leading-relaxed">
              <p className="text-lg">{introductionData?.description}</p>
            </div>
